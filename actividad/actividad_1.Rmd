...
Los resultados de esta ejecución se encuentran en el archivo `actividad/prueba_facil_2017-12-10.RData`. El objeto `resultados_facil`
...
Se aprecia una relación creciente. La correlación lineal de Pearson obtiene un valor de `r cor(min_iter$prob_mutacion, min_iter$min)`. Y, si aplicamos un modelo de regresión lineal simple

```{r}
summary(lm(formula = min ~ I(prob_mutacion*100), data = min_iter))
```
...
No obstante, en la parte izquierda de la gráfica, se observa cierta tendencia no lineal e, incluso, el valor de fitness para las ejecuciones con probabilidad $0.01$ es ligeramente superior al $0.05$, que podria 
...
Los resultados de esta ejecución se encuentran en el archivo `actividad/tabla_facil_aprox1_2017-12-10.RData`
